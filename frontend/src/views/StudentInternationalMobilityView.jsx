...
export default function StudentInternationalMobilityView() {
    const { id } = useParams();
    ...
    const handleDeleteConfirmation = (confirmed) => {
      if (confirmed) {
        ...
        const { fileId } = deleteConfirmation;
          axiosClient
          .delete(`/${role}/student-international-mobility/view/delete/${fileId}`)
          .then(() => {
    ...
    const fetchFiles = (page, setFiles, setFilesTotalPages) => {
      ...
      axiosClient
        .get(`/${role}/student-international-mobility/view/${id}/files?page=${page}`)
        .then(({ data }) => {
    ...
    const fetchHistory = (page, searchQuery, setHistory, setHistoryTotalPages) => {
      ...
      axiosClient
        .get(`/${role}/student-international-mobility/view/${id}/history?page=${fetchPage}&search=${searchQuery}`)
        .then(({ data }) => {
    ...
    useEffect(() => {
        axiosClient
        .get(`/${role}/student-international-mobility/view/${id}/files`)
        .then(({ data }) => {
          setFiles(data.files.data);
          setFilesTotalPages(data.files.last_page);
        ...
        axiosClient
        .get(`/${role}/student-international-mobility/view/${id}/history`)
        .then(({ data }) => {
          setHistory(data.history.data);
          setHistoryTotalPages(data.history.last_page);
          setReloadTable(false); // Reset reloadTable after fetching initial files
        ...
        axiosClient.get(`/${role}/student-international-mobility/view/${id}`).then(res => {
            if(res.data.status === 200)
            {
            ...
            else if (res.data.status === 404)
            {
                ...
                navigate(`/${role}/student-international-mobility`);
            }
        }).catch(error => {
    ...
    const onSubmitForm = (ev) => {
        ...
        .then((res) => {
          if (res.data.success) {
            ...
            axiosClient
            .post(`/${role}/student-international-mobility/update/${id}`, updatedLogRequest, {
            headers: {
            ...
            if (res.data.status === 200) {
                ...
                axiosClient
                .get(`/${role}/student-international-mobility/view/${id}`)
                .then((res) => {
                    if (res.data.status === 200) {
                        const log = res.data.log;
                        const formattedCreatedDate = log.date_received_ovcia
            ...
            {
              swal("Error", res.data.message, "error");
              navigate(`/${role}/student-international-mobility/view/${id}`);
            }
            else
            {
    ...
    const onSubmitFile = (ev) => {
        ...
        axiosClient
        .post(`/${role}/student-international-mobility/add-file`, formData, {
        headers: {
            'Content-Type': 'multipart/form-data',
        },
        })
