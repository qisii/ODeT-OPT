...
export default function ProjectOfficeManagementMobility() {
  ...
  const fetchLogs = (page, statusFilter, searchQuery, dateFilter, setLogs, setTotalPages) => {
    ...
    axiosClient
      .get(`/${role}/project-office-management-mobility?page=${fetchPage}&status=${statusFilter}&search=${searchQuery}&date=${dateFilter}`)
      .then(({ data }) => {
  ...
  useEffect(() => {
    ...
    const onSubmit = (ev) => {
      ev.preventDefault();
      axiosClient
        .post(`/${role}/project-office-management-mobility/add-log-request`, log)
        .then((response) => {
          ...
          axiosClient
            .get(`/${role}/project-office-management-mobility`)
            .then(({ data }) => {
              setLogs(data.logs.data);
              setTotalPages(data.logs.last_page);
              setTotalCount(data.logs.total); // Set the total count
...
useEffect(() => {
  ...
  axiosClient
    ...
    return (
        <PageComponent title="Project Request Office Management"
            buttons={( 
            ...
            <p className="text-sm font-medium text-gray-500 pb-4">
                <span className="inline-flex items-center">
                    ...
                    This contains the requests for Project Request Office Management
                </span>
            </p>
            ...
            <table className="w-full divide-y divide-gray-200 bg-white p-6 mx-auto rounded-lg overflow-hidden">
                ...
                <tr>
                  ...
                  logs.map((log) => (
                    ...
                    onEditClick={() => {
                      if (role === 'admin') {
                        navigate(`/admin/project-office-management-mobility/view/${log.id}`);
                      } else if (role === 'user') {
                        navigate(`/user/project-office-management-mobility/view/${log.id}`);
                      }
                    }}
                    />
                  ))
