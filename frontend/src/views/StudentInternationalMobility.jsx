...
export default function StudentInternationalMobility() {
    ...
    const fetchLogs = (page, statusFilter, searchQuery, dateFilter, setLogs, setTotalPages) => {
      ...
      axiosClient
        .get(`/${role}/student-international-mobility?page=${fetchPage}&status=${statusFilter}&search=${searchQuery}&date=${dateFilter}`)
        .then(({ data }) => {
    ...
    useEffect(() => {
      ...
      const onSubmit = (ev) => {
        ev.preventDefault();
        axiosClient
          .post(`/${role}/student-international-mobility/add-log-request`, log)
          .then((response) => {
            ...
            axiosClient
              .get(`/${role}/student-international-mobility`)
              .then(({ data }) => {
                setLogs(data.logs.data);
                setTotalPages(data.logs.last_page);
                setTotalCount(data.logs.total); // Set the total count
  ...
  useEffect(() => {
    ...
    return (
        <PageComponent title="Students & International Students"
            buttons={( 
            ...
            <p className="text-sm font-medium text-gray-500 pb-4">
                <span className="inline-flex items-center">
                    ...
                    This contains the requests from Student and International Students
                </span>
            </p>
            ...
            <table className="w-full divide-y divide-gray-200 bg-white p-6 mx-auto rounded-lg overflow-hidden">
                ...
                <tr>
                  ...
                  logs.map((log) => (
                    ...
                    onEditClick={() => {
                      if (role === 'admin') {
                        navigate(`/admin/student-international-mobility/view/${log.id}`);
                      } else if (role === 'user') {
                        navigate(`/user/student-international-mobility/view/${log.id}`);
                      }
                    }}
                    />
                    ))
