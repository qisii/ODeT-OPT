...
export default function InternalOfficeProcessMobility() {
  ...
  const fetchLogs = (page, statusFilter, searchQuery, dateFilter, setLogs, setTotalPages) => {
    ...
    axiosClient
      .get(`/${role}/internal-office-process-mobility?page=${fetchPage}&status=${statusFilter}&search=${searchQuery}&date=${dateFilter}`)
      .then(({ data }) => {
  ...
  useEffect(() => {
    ...
    const onSubmit = (ev) => {
      ev.preventDefault();
      axiosClient
        .post(`/${role}/internal-office-process-mobility/add-log-request`, log)
        .then((response) => {
          ...
          axiosClient
            .get(`/${role}/internal-office-process-mobility`)
            .then(({ data }) => {
              setLogs(data.logs.data);
              setTotalPages(data.logs.last_page);
              setTotalCount(data.logs.total); // Set the total count
...
useEffect(() => {
  ...
  axiosClient
    ...
    return (
        <PageComponent title="Internal Office Process"
            buttons={( 
            ...
            <p className="text-sm font-medium text-gray-500 pb-4">
                <span className="inline-flex items-center">
                    ...
                    This contains the requests for Internal Office Process
                </span>
            </p>
            ...
            <table className="w-full divide-y divide-gray-200 bg-white p-6 mx-auto rounded-lg overflow-hidden">
                ...
                <tr>
                  ...
                  logs.map((log) => (
                    ...
                    onEditClick={() => {
                      if (role === 'admin') {
                        navigate(`/admin/internal-office-process-mobility/view/${log.id}`);
                      } else if (role === 'user') {
                        navigate(`/user/internal-office-process-mobility/view/${log.id}`);
                      }
                    }}
                    />
                  ))
