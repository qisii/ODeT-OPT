...
const navigation = [
  { name: 'Dashboard', to: '/user/dashboard' },
  {
    name: 'Requests',
    items: [
      { name: 'Faculty & Admin Staff', to: '/user/faculty-admin-staff-mobility' },
      { name: 'Students & International Students', to: '/user/student-international-mobility' },
      { name: 'Internal Office Process', to: '/user/internal-office-process-mobility' },
      { name: 'Project Request Office Management', to: '/user/project-office-management-mobility' },
    ],
  },
  { name: 'Forms', to: '/user/forms' },
  { name: 'Offices', to: '/user/offices' },
...
export default function StaffLayout() {
  ...
  useEffect(() => {
    ...
    const requestLinks = navigation.find((item) => item.name === 'Requests');
    if (requestLinks) {
                  ...
                  >
                    <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                      {userNavigation.map((item) => (
                        ...
                        {({ active }) => (
                          <NavLink
                          to={`/user/profile/${currentUserId}`} // Pass the userId as a route parameter
                          onClick={(ev) => {
                            if (item.to === '/user/profile') {
                              setActiveRequest(null);
                              navigate(`/user/profile/${currentUserId}`); // Navigate to the profile route using navigate function
                              } else {
                                logout(ev);
                              }
                            }}                              
                            className={classNames(
                              ...
                              (item.to === '/user/profile' && location.pathname === '/user/profile') || (item.to !== '/user/profile' && active)
